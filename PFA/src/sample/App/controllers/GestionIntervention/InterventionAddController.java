...
public class InterventionAddController implements Initializable {
    @FXML
    ...
    @FXML
    private Label IdInterventionField;
...
//            }
                ...
                for (int i=0;i<table_info_Mat.getItems().size();i++){
                    ...
                    for(int i=0;i<table_info_Mat.getItems().size();i++){
                        try {
                            ...
                            rs1.setString(1,table_info_Mat.getItems().get(i).getId());
                            ...
                            if(text.getText()!=null&&isNumeric(text.getText())&&Integer.valueOf(text.getText())>0){
                                if(Integer.valueOf(text.getText())>qtelist.get(this.getTableRow().getIndex())){
                                    ...
                                    try {
                                        ...
                                        String place2=table_info_Mat.getItems().get(this.getTableRow().getIndex()).getId();
                                        ...
                                        rs1.setString(1,table_info_Mat.getItems().get(this.getTableRow().getIndex()).getId());
                                        rs1.setString(2,IdInterventionField.getText());
                                        rs1.setInt(3, Integer.parseInt(text.getText()));
                                        rs1.execute();}
                                        else
                                        {
                                            rs1 = connection.prepareStatement("UPDATE interMAT set qteUsed=? where IDMAT='" + IdInterventionField.getText() + "' AND ID='"+table_info_Mat.getItems().get(this.getTableRow().getIndex()).getId()+"'");
    ...
    void handleClicksTabVehicule(ActionEvent event) throws IOException {
        Parent root = FXMLLoader.load(getClass().getResource("../../view/interIAddEngin.fxml"));
        Scene scene = new Scene(root);
        Stage stage = new Stage();
        stage.setScene(scene);
        stage.initStyle(StageStyle.TRANSPARENT);
