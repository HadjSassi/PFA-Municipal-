...
//        gInterfaceController.getAnchorpane3().getChildren().setAll(view);
    ...
    void handleClicksDeleteSelected(ActionEvent event) throws URISyntaxException {
        String s="";
        String s1 = null;
        ObservableList<Intervention>  ob = table_info.getSelectionModel().getSelectedItems();
        if (ob.toArray().length != 0) {
            for (Intervention per : ob){
                s+=per.getIdI()+"///";
                s1+=per.getIdI();
            }
            ...
            alert.setContentText("Etes-vous sure de supprimer INtervention n°: ///"+s);
...
//        col_select.setCellFactory(cellfact);
    ...
    private void loadData(){
        try {
            Connection connection= getOracleConnection();
            PreparedStatement rs1 =connection.prepareStatement("update INTERVENTION set ETAT='EnCours' where ? BETWEEN dateD and dateF");
            rs1.setDate(1, java.sql.Date.valueOf(LocalDate.now()));
            rs1.execute();
            rs1 =connection.prepareStatement("update INTERVENTION set ETAT='Terminé' where ? > dateF");
            rs1.setDate(1, java.sql.Date.valueOf(LocalDate.now()));
            rs1.execute();}
